...
def vis_detections(im, class_name, dets, thresh=0.1):
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        if score > thresh:
            ...
            text_width, text_height = cv.getTextSize('{:s}: {:.3f}'.format(class_name, score), cv.FONT_HERSHEY_PLAIN, fontScale=1, thickness=1)[0]
            box_coords = ((bbox[0], bbox[1]+15), (bbox[0] + text_width + 2, bbox[1] + 15 - text_height - 2))
            cv.rectangle(im, box_coords[0], box_coords[1], (255, 255, 255), cv.FILLED)
            cv.putText(im, '%s: %.3f' % (class_name, score), (bbox[0], bbox[1] + 15), cv.FONT_HERSHEY_PLAIN, 1.0, (0, 0, 255), thickness=1)
    return im
