    ...
    for img in image_batch:
        ...
        if is_training:
            if cfg.TRAIN.USE_SS_GTBOXES:
                ...
                #shuffle_index = np.random.permutation(len(blob['ss_boxes']))
                #blob['ss_boxes'] = blob['ss_boxes'][shuffle_index, :]
                ss_gt_boxes = torch.from_numpy(blob['ss_boxes'])
                real_gt_boxes = torch.from_numpy(blob['gt_boxes'])
                data, im_info, gt_boxes, real_gt_boxes = crop_image(data, im_info, ss_gt_boxes, img_id, real_gt_boxes)
            else:
...
def get_image_blob(img):
    ...
    #ss_boxes = np.zeros((len(selected_box_indices), 4 + cfg.NUM_CLASSES), dtype=np.float32)
    #ss_boxes[:, 0:4] = img['ss_boxes'][selected_box_indices, :] * im_scale
