...
class ImageDataset(Dataset):
    ...
    def _load_image_data(self):
        image_data = []
        cache_file = os.path.join(self.cache_path, self.name + '_imdb.pkl')
        if os.path.exists(cache_file):
            with open(cache_file, 'rb') as fid:
                image_data = pickle.load(fid)
            print('Data for {} imdb loaded from {}'.format(self.name, cache_file))
        else:
            image_data = [self._load_annotation(idx, id) for idx, id in enumerate(self.image_index)]
            image_data = self._load_object_proposals(image_data)

            with open(cache_file, 'wb') as fid:
                pickle.dump(image_data, fid, pickle.HIGHEST_PROTOCOL)
            print('Wrote imdb to {}'.format(cache_file))
